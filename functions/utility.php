...
function theme_posts_pagination() {
    ...
    if (!empty($paginate_links)) {
        echo '<ul class="cm-paginate">';
        foreach($paginate_links as $k => $paginate_link) {
            if ($k == 0 && strpos($paginate_link, 'prev') == false) {
                echo sprintf('<li class="cm-paginate__ctrl"><span>PREV</span></li>');
            }
            if (strpos($paginate_link, 'current') !== false) {
                echo sprintf('<li class="cm-paginate__page is-current"><span>%s</span></li>', $paginate_link);
            } elseif(strpos($paginate_link, 'prev') !== false) {
                echo sprintf('<li class="cm-paginate__ctrl prev">%s</li>', $paginate_link);
            } elseif(strpos($paginate_link, 'next') !== false) {
                echo sprintf('<li class="cm-paginate__ctrl next">%s</li>', $paginate_link);
            } else {
                echo sprintf('<li class="cm-paginate__page">%s</li>', $paginate_link);
            }
        }
        if (strpos($paginate_link, 'next') == false) {
            echo sprintf('<li class="cm-paginate__ctrl"><span>NEXT</span></li>');
        }
        echo '</ul>';
    }
}
